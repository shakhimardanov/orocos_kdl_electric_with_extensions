...
int ChainIdSolver_Constraint_Vereshchagin::CartToJnt(const JntArray &q, const JntArray &q_dot, JntArray &q_dotdot, Twists& x_dotdot, Jacobian& alfa, const JntArray& beta, const Wrenches& f_ext, JntArray &torques)
{
    //Check sizes always
    ...
    this->final_upwards_sweep(q_dotdot, x_dotdot, torques);
}
...
void ChainIdSolver_Constraint_Vereshchagin::downwards_sweep(Jacobian& alfa, JntArray &torques)
{
    unsigned int j = nj - 1;
    for (int i = ns; i >= 0; i--)
    {
...
void ChainIdSolver_Constraint_Vereshchagin::final_upwards_sweep(JntArray &q_dotdot, Twists& x_dotdot, JntArray &torques)
{
    unsigned int j = 0;
    ...
    for (unsigned int i = 1; i <= ns; i++)
    {
        ...
        double parentAccComp = -dot(s.Z, parent_force) / s.D;
...
#ifdef DEBUGGER
        printf("In final sweep:   joint#   internal torque,  constrain torque proj  %d           %f         %f\n",  j, torques(j), constraint_torque);
        std::cout << "parent_force = " << parent_force << std::endl;
        //std::cout<<constraint_force<<std::endl;
        std::cout << constraint_torque << std::endl;
        printf("Joint#        generalized force      s.R              s.PC        internal torque in final sweep          Constrain torque proj.\n");
        printf("%d           %f                   %f        %f                %f                   %f\n", j, s.u, dot(s.Z, s.R), dot(s.Z, s.PC), torques(j), torque_local);
#endif DEBUGGER
...
void ChainIdSolver_Constraint_Vereshchagin::getLinkCartesianPose(Frames& x_base)
{
    for (int i = 0; i < ns; i++)
    {
...
void ChainIdSolver_Constraint_Vereshchagin::getLinkPose(Frames& x_local)
{
    for (int i = 0; i < ns; i++)
    {
...
void ChainIdSolver_Constraint_Vereshchagin::getLinkCartesianVelocity(Twists& xDot_base)
{
    for (int i = 0; i < ns; i++)
    {
        xDot_base[i] = results[i].v;
    }
    return;
}

void ChainIdSolver_Constraint_Vereshchagin::getJointBiasAcceleration(JntArray& bias_q_dotdot)
{
    for (int i = 0; i < ns; i++)
    {
        //this is only force
...
void ChainIdSolver_Constraint_Vereshchagin::getJointConstraintAcceleration(JntArray& constraint_q_dotdot)
{
    for (int i = 0; i < ns; i++)
    {
...
void ChainIdSolver_Constraint_Vereshchagin::getJointNullSpaceAcceleration(JntArray& nullspace_q_dotdot)
{
    for (int i = 0; i < ns; i++)
    {
...
void ChainIdSolver_Constraint_Vereshchagin::getLinkBiasForceAcceleratoinEnergy(Eigen::VectorXd& G)
{
 for (int i = 0; i < ns; i++)
    {
        G = results[i + 1].G;
...
void ChainIdSolver_Constraint_Vereshchagin::getLinkBiasForceMatrix(Wrenches& R_tilde)
{
    for(int i = 0; i < ns; i++)
    {
...
void ChainIdSolver_Constraint_Vereshchagin::getLinkUnitForceAccelerationEnergy(Eigen::MatrixXd& M)
{
...
void ChainIdSolver_Constraint_Vereshchagin::getLinkUnitForceMatrix(Matrix6Xd& E_tilde)
{
