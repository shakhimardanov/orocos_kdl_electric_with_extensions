...
int main()
{
   ...
   else
   {
    ...
    jointFinalPose(2) = M_PI / 6.0;
    jointFinalPose(3) = M_PI / 12.0;
    jointFinalPose(4) = M_PI / 18.0;

    JntArray jointInitialPose(numberOfJoints);
    jointInitialPose(0) = 0.0;
    jointInitialPose(1) = 0.0;
    jointInitialPose(2) = 0.0;
    jointInitialPose(3) = 0.0;
    ...
    //-------------------------------------------------------------------------------------//
    double taskTimeConstant = 0.2; //Time required to complete the task move(frameinitialPose, framefinalPose) default T=10.0
    ...
    /*
    double b0_y = 0.58636; //should come from initial joint configuration
    double b1_y = 0.0;
    double b2_y =  ((0.1 - b0_y)*3.0 / (simulationTime * simulationTime)); //yFinal = 0.1
    double b3_y = -((0.1 - b0_y)*2.0 / (simulationTime * simulationTime * simulationTime));

    double b0_x = 0.44992; //should come from initial joint configuration
    double b1_x = 0.0;
    double b2_x = ((0.44992 - b0_x)*3.0 / (simulationTime * simulationTime)); // Xfinal= xInitial = 0.44992
    double b3_x = -((0.44992 - b0_x)*2.0 / (simulationTime * simulationTime * simulationTime));
        ...
        */
        ...
        if (status == 0)
        {
            for (unsigned int i = 0; i < numberOfJoints; i++)
            {
                ...
                //printf("Actual joint values: %f    %d      %f       %f     %f\n", t, i, jointPoses[0](i), jointRates[0](i), jointAccelerations[0](i));
            }
            //constraintSolver.initial_upwards_sweep(jointPoses[0], jointRates[0], jointAccelerations[0], externalNetForces);
             constraintSolver.getLinkCartesianPose(cartX[0]);
            //constraintSolver.getLinkPose(cartX[0]);
            cartX[0][0].M.GetRPY(roll,pitch,yaw);
            printf("Link1 %f      %f          %f          %f        %f          %f          %f\n", t, cartX[0][0].p.x(), cartX[0][0].p.y(), cartX[0][0].p.z(), roll, pitch, yaw);
            cartX[0][1].M.GetRPY(roll,pitch,yaw);
            printf("Link2 %f      %f          %f          %f        %f          %f          %f\n", t, cartX[0][1].p.x(), cartX[0][1].p.y(), cartX[0][1].p.z(), roll, pitch, yaw);
            cartX[0][2].M.GetRPY(roll,pitch,yaw);
            printf("Link3 %f      %f          %f          %f        %f          %f          %f\n", t, cartX[0][2].p.x(), cartX[0][2].p.y(), cartX[0][2].p.z(), roll, pitch, yaw);
            cartX[0][3].M.GetRPY(roll,pitch,yaw);
            printf("Link4 %f      %f          %f          %f        %f          %f          %f\n", t, cartX[0][3].p.x(), cartX[0][3].p.y(), cartX[0][3].p.z(), roll, pitch, yaw);
        }
        ...
        //betha(1) = alpha(1, 1)*((Kp[1]) * cartXDotDot[2][1].vel[1] + (Kv[0]) * cartXDot[2][1].vel[1] + (Kp[1]) * cartX[2][1].p[1] + Ki[1] * cartX[3][1].p[0]);
