...
namespace KDL
{
  ...
  void ChainIdSolver_Vereshchagin::initial_upwards_sweep(const JntArray &q, const JntArray &qdot, const JntArray &qdotdot, const Wrenches& f_ext)
  {
	...
	q_=qdot_=qdotdot_=0.0;
      ...
      s.F = segment.pose(q_); //X pose of each link in link coord system
      ...
      Twist vj = s.F.M.Inverse(segment.twist(q_, qdot_)); //XDot of each link
      Twist aj = s.F.M.Inverse(segment.twist(q_, qdotdot_)); //XDotDot of each link

      //The unit velocity due to the joint motion of the segment expressed in the segments reference frame (tip)
      s.Z = s.F.M.Inverse(segment.twist(q_, 1.0));
  ...
  void ChainIdSolver_Vereshchagin::downwards_sweep(const Jacobian& alfa, const JntArray &torques)
  {
    ...
    for (int i = ns; i >= 0; i--)
    {
      ...
      if (chain.getSegment(i - 1).getJoint().getType() != Joint::None && i>1) //when at least 1 segment there
	j--;
  ...
  void ChainIdSolver_Vereshchagin::final_upwards_sweep(JntArray &q_dotdot, JntArray &torques)
  {
    ...
    {
      if (chain.getSegment(i - 1).getJoint().getType() != Joint::None && i>1)
	j++;
      segment_info& s = results[i];
	...
	a_p = results[i - 1].acc;
      ...
      //20.04.12 modified to return full joint torques. This is needed to work in non-constraint mode
      torques(j) = s.u + parent_forceProjection;
      //To return constraint torques uncomment next line
