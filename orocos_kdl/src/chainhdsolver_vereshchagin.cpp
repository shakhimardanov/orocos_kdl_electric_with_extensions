...
int ChainHdSolver_Vereshchagin::CartToJnt(const JntArray &q, const JntArray &q_dot, JntArray &q_dotdot, const Jacobian& alfa, const JntArray& beta, const JntArray& betaControl, const Wrenches& f_ext, JntArray &torques, JntArray &controltorques)
{
    //Check sizes always
...
void ChainHdSolver_Vereshchagin::constraint_calculation(const JntArray& beta)
{
    ...
    // std::cout<<"E4: "<<results[4].E_tilde.transpose()<<std::endl;
    ...
    //The sum below should never be done in the algorithm. It is part of the control
...
void ChainHdSolver_Vereshchagin::final_upwards_sweep(JntArray &q_dotdot, JntArray &torques, JntArray &controltorques)
{
    unsigned int j = 0;
    Twist a_g;
    Twist a_p;
    ...
    for (unsigned int i = 1; i <= ns; i++)
    {
        ...
        s.nullspaceAccComp = (s.u + controltorques(j))/ s.D;
        ...
        a_Posture = s.F.Inverse(a_p + s.Z * (s.nullspaceAccComp + parentAccComp) + s.C);
        a_Constraint = s.F.Inverse(s.Z*s.constAccComp);
        s.acc = a_Posture + a_Constraint;
        ...
        if(i==ns)
        {
            ...
            // std::cout << "accPosture "<< i << "   " << accPosture << std::endl;
        }
...
void ChainHdSolver_Vereshchagin::constraint_update(const JntArray& betaControl)
{
...
void ChainHdSolver_Vereshchagin::final_sweep_update(JntArray &q_dotdot, JntArray &torques)
{
        unsigned int j = 0;
    Twist a_p;
    Twist a_Posture;
...
void ChainHdSolver_Vereshchagin::getLinkUnitForceAccelerationEnergy(Eigen::MatrixXd& M)
{
...
void ChainHdSolver_Vereshchagin::getLinkUnitForceMatrix(Matrix6Xd& E_tilde)
{
