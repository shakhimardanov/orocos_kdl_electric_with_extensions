...
void TreeIdSolver_Vereshchagin::initial_upwards_sweep(const JntArray &q, const JntArray &qdot, const JntArray &qdotdot, const Wrenches& f_ext)
{
    ...
    //first element should be root : this relies on weak ordering assumption of the maps and does it really matter?
    for (SegmentMap::const_iterator it = tree.getSegments().begin(); it != tree.getSegments().end(); it++)
    {
        const TreeElement parentElement = it->second;
        std::cout << "Parent element name in current iteration " << parentElement.segment.getName() << std::endl;
        std::cout << "Current/parent joint index and value " << parentElement.q_nr << " " << q(parentElement.q_nr) << std::endl;


        for (std::vector<SegmentMap::const_iterator>::const_iterator childIter = it->second.children.begin(); childIter != it->second.children.end(); childIter++)
            //for(unsigned int i = 0 ; i < it->second.children.size(); i++)
        {
            //use feature of KDL tree joint numbering
            //when creating a tree through addsegment every iteration starting from
            ...
            const TreeElement childElement = (*childIter)->second; //it->second.children[i]->second;
            segment_info& childElementState = segmentResults[(childElement.q_nr)];

            childElementState.segmentName = childElement.segment.getName();
            std::cout << "Element's child name in of current segment " << childElementState.segmentName << std::endl;
            //std::cout << "Child iteration number " << i << std::endl;
            childElementState.F = childElement.segment.pose(q(childElement.q_nr));
            childElementState.F_base = segmentResults[parentElement.q_nr].F_base * childElementState.F;

            std::cout << "Child joint index and value " << childElement.q_nr << " " << q(childElement.q_nr) << std::endl;
            std::cout << "F link local:  " << std::endl << childElementState.F << std::endl;
            std::cout << "F link base:  " << std::endl << childElementState.F_base << std::endl;
            std::cout << "F parentlink base:  " << std::endl << segmentResults[parentElement.q_nr].F_base << std::endl;
            ...
            vj = childElementState.F.M.Inverse(childElement.segment.twist(q(childElement.q_nr), qdot(childElement.q_nr)));
            aj = childElementState.F.M.Inverse(childElement.segment.twist(q(childElement.q_nr), qdot(childElement.q_nr)));

            //The unit velocity due to the joint motion of the segment expressed in the segments reference frame (tip)
            childElementState.Z = childElementState.F.M.Inverse(childElement.segment.twist(q(childElement.q_nr), 1.0));
            //Put Z in the joint root reference frame:
            childElementState.Z = childElementState.F * childElementState.Z;

            //The total velocity of the segment expressed in the the segments reference frame (tip)
            childElementState.v = childElementState.F.Inverse(segmentResults[(parentElement.q_nr)].v) + vj; // Azamat: recursive velocity of each link in segment frame
            childElementState.A = childElementState.F.M.Inverse(segmentResults[(parentElement.q_nr)].A);

            //  if this child's parent is root element overwrite calculated velocity and acc with the values below.
            //    for children of the tree root
            if (it == tree.getRootSegment())
            {

                std::cout << "root joint index and value " << parentElement.q_nr << " " << q(parentElement.q_nr) << std::endl;
                childElementState.v = vj;
                childElementState.A = childElementState.F.M.Inverse(acc_root);
                childElementState.F_base = childElementState.F;
                std::cout << "I am the root F_base  " << std::endl << childElementState.F_base << std::endl;
                std::cout << "I am the root F_local " << std::endl << childElementState.F << std::endl;
            }


            //c[i] = cj + v[i]xvj (remark: cj=0, since our S is not time dependent in local coordinates)
            //The velocity product acceleration
            std::cout << childElementState.segmentName << " Initial upward " << childElementState.v << std::endl;

            childElementState.C = childElementState.v*vj; //This is a cross product Azamat: cartesian space BIAS acceleration in local link coord.

            //s.A+=s.C;
            std::cout << "Child element" << childElementState.segmentName << " acceleration: " << childElementState.F_base.M * childElementState.A << std::endl;

            //Put C in the joint root reference frame
            childElementState.C = childElementState.F * childElementState.C; //+F_total.M.Inverse(acc_root));
            //The rigid body inertia of the segment, expressed in the segments reference frame (tip) (link local)
            childElementState.H = childElement.segment.getInertia();
            ...
            Wrench FextLocal = childElementState.F_base.M.Inverse() * f_ext[parentElement.q_nr];
            childElementState.U = childElementState.v * (childElementState.H * childElementState.v) - FextLocal; //f_ext[i];
